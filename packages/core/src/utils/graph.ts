...
export function nodeToRect(node: GraphNode): Rect {
  return {
    ...
    width: node.dimensions.width || (typeof node.width === 'number' ? node.width : 0),
    height: node.dimensions.height || (typeof node.height === 'number' ? node.height : 0),
  }
}
...
export function getNodesInside(
  ...
  for (const node of nodes) {
    ...
    const width = dimensions.width || (typeof node.width === 'number' ? node.width : null)
    const height = dimensions.height || (typeof node.height === 'number' ? node.height : null)
